...
const getEvents = async () => {
  ...
  const eventInfos = await page.evaluate(() => {
    ...
    const eventInfosList = document.querySelectorAll(
      ".community-events_ecs_category"
    );
    ...
    return Array.from(eventInfosList).map((eventInfo) => {
      ...
      const eventDate = eventInfo.querySelector(
  ...
  for (i = 0; i < eventInfos.length; i++) {
    console.log(
      "\t{\n" +
        '\t\teventDate: "' +
        eventInfos[i]["eventDate"] +
        '",\n' +
        '\t\teventName: "' +
        eventInfos[i]["eventName"] +
        '",\n' +
        '\t\teventLocation: "' +
        eventInfos[i]["eventLocation"] +
        '"\n' +
        "\t},\n"
    );
  }
